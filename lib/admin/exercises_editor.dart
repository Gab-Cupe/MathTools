...
class _ExerciseEditorScreenState extends State<ExerciseEditorScreen> {
  ...
  Future<void> _saveExercise() async {
    ...
    try {
      final ejercicioToSave = {
        ...
        'solucionImagenUrl':
            _ejercicioData['solucionImagenUrl'], // Asegúrate de incluir este campo
      };

      await _dbRef
          .child(
            'ejercicios/$_selectedTema/$_selectedSubtema/${_ejercicioData['id']}',
          )
  ...
  Future<void> _deleteTopic() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '¿Estás seguro de eliminar el tema "${_selectedTema}"? Se perderán todos sus subtemas y ejercicios.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _addNewSubTopic() async {
    ...
    try {
      ...
      await _dbRef.child('subtemas/$_selectedTema').set([
        ..._subtemas,
        newSubTopic,
      ]);
  ...
  Future<void> _deleteSubTopic() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '¿Estás seguro de eliminar el subtema "${_selectedSubtema}"? Se perderán todos sus ejercicios.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
    ...
    try {
      ...
      final updatedSubTopics = _subtemas
          .where((s) => s != _selectedSubtema)
          .toList();
      await _dbRef.child('subtemas/$_selectedTema').set(updatedSubTopics);

      // Eliminar toda la estructura del subtema
      await _dbRef
          .child('ejercicios/$_selectedTema/$_selectedSubtema')
          .remove();
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final isLargeScreen =
            constraints.maxWidth > 800; // Define tu breakpoint
        ...
        if (isLargeScreen) {
          ...
          return Row(
            ...
            children: [
              ...
              Expanded(flex: 2, child: _buildExerciseListPanel()),
              ...
              Expanded(
                ...
                child: _ejercicioData.isNotEmpty
                    ...
                    : const Center(
                        child: Text(
                          'Selecciona o crea un ejercicio para editar',
                        ),
                      ),
              ),
            ],
            ...
            children: [
              ...
              Expanded(
                child: _ejercicios.isEmpty
                    ? const Center(
                        child: Text('No hay ejercicios en este subtema'),
                      )
                    : ListView.builder(
          ...
          children: [
            Expanded(
              child: DropdownButtonFormField<String>(
                initialValue: _selectedTema,
                items: _temas.map((tema) {
                  return DropdownMenuItem(value: tema, child: Text(tema));
                }).toList(),
                onChanged: (value) {
                  if (value != null) {
            ...
            IconButton(
              icon: Icon(
                _showNewTopicField ? Iconsax.close_circle : Iconsax.add,
              ),
              onPressed: () {
                setState(() {
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: DropdownButtonFormField<String>(
                initialValue: _selectedSubtema,
                items: _subtemas.map((subtema) {
                  return DropdownMenuItem(value: subtema, child: Text(subtema));
                }).toList(),
                onChanged: (value) {
            ...
            IconButton(
              icon: Icon(
                _showNewSubTopicField ? Iconsax.close_circle : Iconsax.add,
              ),
              onPressed: () {
                if (_selectedTema == null) return;
                setState(() {
          ...
          child: Column(
            children: [
              ...
              DropdownButtonFormField<String>(
                initialValue: _ejercicioData['dificultad'] ?? 'facil',
                items: const [
  ...
  Widget _buildImageUploader() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        if (_ejercicioData['imagenUrl'] != null ||
  ...
  Widget _buildSolutionImageUploader() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        if (_ejercicioData['solucionImagenUrl'] != null ||
            (!kIsWeb && _imageFile != null))
          Container(
            height: 200,
            decoration: BoxDecoration(
            ...
            child: ClipRRect(
              ...
              child: _isUploading
                  ...
                  : kIsWeb
                  ? Image.network(
                      _ejercicioData['solucionImagenUrl'],
                      fit: BoxFit.cover,
                    )
                  : _imageFile != null
                  ? Image.file(_imageFile!, fit: BoxFit.cover)
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            icon: const Icon(Iconsax.gallery_add),
            label: Text(
              _isUploading ? 'Subiendo...' : 'Seleccionar imagen para solución',
            ),
            onPressed: _isUploading
                ? null
                : () => _pickAndUploadSolutionImage(),
          ),
        ),
      ],
    );
      ...
      return Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: _buildOptionTextField(key, options[key]?.toString() ?? ''),
            ),
            const SizedBox(width: 12),
