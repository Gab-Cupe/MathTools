...
class PuntosService {
  ...
  Future<void> actualizarPuntos(String userId, int nuevosPuntos) async {
    await _dbRef.child('users/$userId').update({'points': nuevosPuntos});
  ...
  Future<void> addPuntos(String userId, int addPuntos) async {
    ...
    if (snapshot.exists) {
      ...
      await userRef.update({'points': currentPoints + addPuntos});
  ...
  Stream<List<UserModel>> get rankingGlobalStream {
    return _dbRef.child('users').orderByChild('global_rank').onValue.map((
      event,
    ) {
  ...
  Future<void> actualizarTodosLosRanks() async {
    try {
      ...
      final snapshot = await _dbRef.child('users').orderByChild('points').get();
      ...
      final sortedUsers = usersMap.entries.toList()
        ..sort(
          (a, b) =>
              (b.value['points'] as num).compareTo(a.value['points'] as num),
        );
